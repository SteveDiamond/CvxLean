#!/usr/bin/env python3
"""
Fixed CVXPY to CVXLean Integration Tool

This generates proper CVXLean syntax that actually works with the framework.
"""

import cvxpy as cp
from cvxpy_to_lean_json import problem_to_cvxlean_json
from fixed_json_to_lean import fixed_json_to_lean_code
import os
from typing import Optional


def fixed_cvxpy_to_lean_code(problem: cp.Problem, prob_name: str) -> str:
    """
    Convert CVXPY problem directly to proper CVXLean Lean code.
    ...
    # Step 2: Convert JSON to proper Lean
    lean_code = fixed_json_to_lean_code(json_str)
    
    return lean_code


def fixed_cvxpy_to_lean_file(problem: cp.Problem, filename: str, prob_name: str) -> str:
    """
    ...
    """
    lean_code = fixed_cvxpy_to_lean_code(problem, prob_name)
...
def generate_fixed_examples():
    ...
    fixed_cvxpy_to_lean_file(lp_problem, "FixedSimpleLP.lean", "simple_lp")
    ...
    fixed_cvxpy_to_lean_file(qp_problem, "FixedQuadratic.lean", "quadratic_problem")
    ...
    fixed_cvxpy_to_lean_file(portfolio_problem, "FixedPortfolio.lean", "portfolio_optimization")
    
    print("\nGenerated 3 working CVXLean files!")
    print("Files created:")
    for filename in ["FixedSimpleLP.lean", "FixedQuadratic.lean", "FixedPortfolio.lean"]:
        if os.path.exists(filename):
            print(f"  ✓ {filename}")


if __name__ == "__main__":
    print("Fixed CVXPY to CVXLean Integration Tool")
    print("=" * 50)
